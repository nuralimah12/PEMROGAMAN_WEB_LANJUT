                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            ...
                            @foreach($barang as $item) 
                              <option value="{{ $item->barang_id }}" @if($item->barang_id == $stok->barang_id) selected @endif>{{ $item->barang_id }}</option> 
                            @endforeach 
                            </select> 
                            @error('barang_id')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            ...
                            @foreach($user as $item) 
                              <option value="{{ $item->user_id }}" @if($item->user_id == $stok->user_id) selected @endif>{{ $item->user_id }}</option> 
                            @endforeach 
                            </select> 
                            @error('user_id')
                                <small class="form-text text-danger">{{ $message }}</small>
